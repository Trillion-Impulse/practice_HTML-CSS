...
### 그밖에 텍스트의 레이아웃과 줄바꿈, 공백처리

#### white-space

- 텍스트 내에서 공백의 처리 방식을 설정
- 축약: `white-space: normal | nowrap | pre | pre-line | pre-wrap | initial | inherit`
- 속성 값
    - normal (기본 값)
        - 연속된 공백을 하나로 처리
        - 텍스트가 부모 요소의 너비를 초과하면 자동으로 줄 바꿈
    - nowrap
        - 연속된 공백을 하나로 처리
        - 자동 줄 바꿈이 일어나지 않음
    - pre
...
- 텍스트의 문자 간 간격(자간)을 설정
- 축약: `letter-spacing: normal | length | initial | inherit`
- 속성 값
    - normal: 기본 값, 문자 간 간격을 기본 상태로 유지
    - length: px, em 등 고정 값으로 지정, 음수 허용

#### word-spacing

- 단어 간의 간격을 설정
- 축약: `word-spacing: normal|length|initial|inherit`
- 속성 값
    - normal: 기본 값, 단어 간 간격을 기본 상태로 유지
    - length: px, em 등 고정 값으로 지정, 음수 허용

#### word-break

- 단어가 라인 끝에 나올 경우 어떻게 처리할지 지정하는 속성
- 단어가 어디에서 줄 바꿈될 수 있을지를 제어
- 텍스트가 부모 요소의 너비를 초과할 때, 단어가 어떻게 처리될지 결정
- 축약: `word-break: normal | break-all | keep-all | initial | inherit`
- 속성 값
    - normal: 기본 값, 일반적인 단어 구분 규칙에 따라 줄 바꿈
        - 중단점은 공백이나 하이픈(-), CJK(한중일)는 음절
    - break-all: 단어 내에서 임의로 줄 바꿈을 허용
        - 중단점은 음절
        - 모든 글자가 요소를 벗어나지 않고 개행
    - keep-all: 단어의 중간에서 줄 바꿈이 일어나지 않음
        - 중단점은 공백이나 하이픈(-)
        - 주로 CJK에서 사용
        - 비 CJK 텍스트 에서는 normal과 동일

#### overflow-wrap

- 이전 명칭: word-wrap
- 텍스트가 부모 요소의 너비를 초과할 때 어떻게 줄 바꿈을 처리할지 제어
- 요소를 벗어난 단어의 줄바꿈을 지정
- 속성 값
    - normal
        - 텍스트 내에서 자연스러운 단어 간 구분을 따름
        - 텍스트가 요소의 너비를 넘으면 줄 바꿈을 하지 않으며, 레이아웃이 깨질 수 있음
        - 긴 단어는 자동으로 줄 바꿈 없이 넘침
    - break-word
        - 텍스트가 부모 요소의 너비를 초과하면, 단어 중간에서 강제로 줄 바꿈을 시도

---

### display

- HTML 요소가 페이지에서 어떻게 표시될지를 정의
- 축약: `display: value`
- 속성 값
    - block: block level 요소처럼 렌더링
        - 화면에서 항상 새로운 줄에서 시작
        - 너비는 기본적으로 부모 요소의 너비에 맞춰 확장
    - inline: inline level 요소처럼 렌더링
        - 요소는 한 줄 알에서 다른 요소와 나란히 표시
        - 다른 인라인 요소들과 동일한 줄에 표시
        - 너비와 높이를 설정
    - inline-block: inline level 요소처럼 렌더링(배치)되지만 block level의 성질을 가짐
        - 인라인 요소처럼 다른 요소들과 한 줄에 나란히 배치
        - 블록 요소처럼 너비와 높이 설정 가능
    - none: 요소가 렌더링 되지 않음
        - 렌더링에서 완전히 제외되므로, 페이지 레이아웃에 영향을 미치지 않음
        - 다른 요소들과 공간을 공유하지 않음
